// fileController delegates to documentController logic
// Kept as a separate route for Cloudinary-backed uploads
const Document = require('../models/Document');

const uploadDocument = async (req, res) => {
  const { employeeId, documentType, fileName, fileUrl, description } = req.body;
  if (!documentType || !fileName) {
    return res.status(400).json({ success: false, message: 'documentType and fileName are required' });
  }
  try {
